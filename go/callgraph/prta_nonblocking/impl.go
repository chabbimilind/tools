...
type rta struct {
	// Use xsync.MapOf internally for concurrent access
	result *Result
...
func (r *rta) addEdge(caller *ssa.Function, site ssa.CallInstruction, callee *ssa.Function, addrTaken bool, workerID int) {
	...
	if g := r.result.callGraph; g != nil {
		...
		g.AddCallGraphEdge(from, site, to)
	}
}
...
func (r *rta) visitInvoke(site ssa.CallInstruction, workerID int) {
	...
	// One add per visitInvoke using TypeMap.Len(); the per-worker
	// counter avoids any cross-worker atomic increment per impl.
...
// RTA is an RTA implementation with lock-free concurrent callgraph.
type RTA struct{}
...
func (r *rta) processNodesInBatches(numWorkers int) {
	...
	// Get the materialized callgraph for processing
	cg := r.result.GetCallGraph()
...
func (r *rta) addRuntimeType(T types.Type, skip bool, workerID int) {
	...
	if _, ok := T.Underlying().(*types.Interface); !ok {
		...
		cinfo.implements.Iterate(func(k types.Type, v any) {
			...
			val, _ := r.invokeSites.LoadOrStore(I, newCallSites())
			s := val.(*callSites)
			// One add per (T, I) using xsync.Map.Size(); the per-worker
			// counter eliminates the per-site atomic increment.
