...
type rta struct {
	...
	// performed inside visitInvoke. Without sitesByMethod, every
	// addInvokeEdge invocation does one lookup — equals len(impls)
	// summed across visitInvoke calls. Baseline value for sitesByMethod
	// savings comparisons (along with [srta]).
	invokeLookupsFromVisitInvoke int
	// invokeLookupsFromAddRuntimeType counts utils.LookupMethodSeq calls
	// performed inside addRuntimeType. One per (T, I, site).
...
func (r *rta) visitAddrTakenFunc(f *ssa.Function) {
	// Create two-level map (Signature -> Function -> struct{}).
	S := f.Signature
